...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		foreach($appointment_data as $appointment_row)
		{
			...
			$html = '
			<h4 class="text-center"> Сведения о пациенте: </h4>
			<table class="table">
			';
			...
			foreach($patient_data as $patient_row)
			{
				$html .= '
				<tr>
					<th width="40%" class="text-right"> Имя пациента: </th>
					<td>'.$patient_row["patient_first_name"].' '.$patient_row["patient_last_name"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right"> Контактный номер: </th>
					<td>'.$patient_row["patient_phone_no"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right"> Адрес: </th>
			...
			<hr />
			<h4 class="text-center"> Сведения о назначении: </h4>
			<table class="table">
				<tr>
					<th width="40%" class="text-right"> Назначение №: </th>
			...
			foreach($doctor_schedule_data as $doctor_schedule_row)
			{
				$html .= '
				<tr>
					<th width="40%" class="text-right"> Имя врача: </th>
					<td>'.$doctor_schedule_row["doctor_name"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right"> Назначенная дата: </th>
					<td>'.$doctor_schedule_row["doctor_schedule_date"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right"> День встречи: </th>
			...
			$html .= '
				<tr>
					<th width="40%" class="text-right"> Время встречи: </th>
					<td>'.$appointment_row["appointment_time"].'</td>
				</tr>
				<tr>
					<th width="40%" class="text-right"> Причина назначения: </th>
			...
			if($appointment_row["status"] != 'Cancel')
			{
				if($_SESSION['type'] == 'Admin')
				{
					if($appointment_row['patient_come_into_hospital'] == 'Yes')
					{
						if($appointment_row["status"] == 'Completed')
						{
							$html .= '
								<tr>
									<th width="40%" class="text-right"> Пациент приходит в госпиталь: </th>
									<td>Yes</td>
								</tr>
								<tr>
									<th width="40%" class="text-right"> Комментарий доктора: </th>
					...
					else
					{
						$html .= '
							<tr>
								<th width="40%" class="text-right"> Пациент приходит в больницу </th>
								<td>
									<select name="patient_come_into_hospital" id="patient_come_into_hospital" class="form-control" required>
										<option value="">Select</option>
				...
				{
					if($appointment_row["patient_come_into_hospital"] == 'Да')
					{
						if($appointment_row["status"] == 'Завершено')
						{
							$html .= '
								<tr>
									<th width="40%" class="text-right"> Комментарий врача: </th>
									<td>
	...
	if($_POST['action'] == 'change_appointment_status')
	{
		if($_SESSION['type'] == 'Admin')
		{
			...
			echo '<div class="alert alert-success"> Изменение статуса встречи на «В процессе» </div>';
		}
		...
		if($_SESSION['type'] == 'Doctor')
		{
			if(isset($_POST['doctor_comment']))
			{
				...
				echo '<div class="alert alert-success"> Назначение завершено </div>';
			}
		}
	}
	...
	if($_POST["action"] == 'delete')
	{
		...
		echo '<div class="alert alert-success"> Расписание врачей удалено. </div>';
	}
}
